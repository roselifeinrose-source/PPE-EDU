          ...
          return (
            <div
              ...
              className={`flex items-center justify-between px-3 py-2 rounded-lg text-sm transition-all duration-200 ${
                isMe ? 'bg-indigo-50 dark:bg-indigo-900/20' : 'hover:bg-slate-50 dark:hover:bg-slate-700/50'
              }`}
            >
              <div className="flex items-center gap-3">
                <div className="w-6 h-6 rounded-full bg-slate-100 dark:bg-slate-700 flex items-center justify-center text-xs font-bold text-slate-600 dark:text-slate-400 shrink-0">
                  {RankIcon ? <RankIcon size={14} className={i === 0 ? 'text-amber-500' : i === 1 ? 'text-slate-400' : 'text-amber-700'} /> : `#${i + 1}`}
                </div>
                <span className={`font-medium ${isMe ? 'text-indigo-700 dark:text-indigo-300' : 'text-slate-700 dark:text-slate-300'}`}>
                  {student.name}
                </span>
              </div>
              <span className={`text-xs font-medium ${isMe ? 'text-indigo-600 dark:text-indigo-400' : 'text-slate-500 dark:text-slate-400'}`}>
                {student.totalXP} XP
              </span>
            </div>
          )
